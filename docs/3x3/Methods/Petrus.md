...
There is a common debate in the community as to the best way to handle edge orientation. The first common way is to solve the 2x2x3 block at db, use `U, R, L, M, F` to orient the edges, then rotate to have the block at dl while solving the remainder of the first two layers. The second common way is to solve and keep the block at dl and use `U, R, F, f, S` to orient the edges. It is often stated that a weakness is the built-in rotation if orienting edges with the block at db. However, it is important to note that the Petrus method doesn’t naturally include a specific rotation just for the edge orientation step. Petrus himself solves the 2x2x3 block and edge orientation with rotations used at any point for ergonomics reasons, then keeps the block at dr while solving the first two layers with the U and L layers free to turn [1].
