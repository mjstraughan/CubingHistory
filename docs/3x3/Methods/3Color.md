...
A note is that with the 3-color corners already solved, the 6-color corners and the 3-color edges can be solved in either order. A disadvantage of solving the 3-color edges first is that edge facelets may end up being slightly misplaced after solving the 6-color corners (or at least reducing them to be half-turn solvable).
