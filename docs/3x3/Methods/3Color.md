...
A note is that with the 3-color corners already solved, the 6-color corners and the 3-color edges can be solved in either order. A disadvantage of solving the 3-color edges first is that some edge facelets may end up being slightly misplaced after solving the 6-color corners (or at least reducing them to be half-turn solvable). This can be avoided by solving the 6-color corners prior to the 3-color edges.
