...
In October 2004 Pochmann improved upon the one algorithm method, evolving it into what is now called “Classic Pochmann” [^pochmann-2004-3] [^pochmann-2004-4]. Instead of a single algorithm, this new method uses four main algorithms taken from the PLL algorithm set: T, Ja, Jb, and a modified T permutation that swaps the UBL and DFR corners and UB and UL edges. An additional algorithm is used in the case of parity at the transition between solving edges and corners. The solver could learn an Ra PLL algorithm to directly fix parity or, as in Pochmann's preference, use the already known T permutation algorithm to swap the corners followed by an easier to learn Ua permutation algorithm to cycle the edges in order to achive the same effect as an Ra permutation. These additional algorithms introduced in this upgrade to the one algorithm method bring versatility to setting up positions and solving pieces.
