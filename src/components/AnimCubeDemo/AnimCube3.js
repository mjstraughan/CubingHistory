...
export default function AnimCube3(params) {
  ...
  function loadConfigFile(file) {
    ...
    function handler() {
      if (xhr.readyState == 4) {
        if (xhr.status == 200) parseConfigFile(xhr.responseText);
        else console.log("Error loading config file: " + file);
        init();
      }
    }
  }
  ...
  function parseConfigFile(text) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (typeof line[1] != "undefined") config[line[0]] = line[1].trim();
    }
  }
  ...
  function init() {
    ...
    if (param != null)
      if ("1" == param) {
        ...
        for (var i = 0; i < 9; i++) cube[0][i] = 22;
        param = getParameter("scw");
        if (param != null) {
          if ("1" == param) scw = 1;
          else if ("2" == param) scw = 2;
        }
        if (scw == 1) colors[10] = "#000000";
      }
      ...
      if (param != null && param.length == 54) {
        for (var i = 0; i < 6; i++) {
          for (var j = 0; j < 9; j++) {
            ...
            for (var k = 0; k < 23; k++) {
              if (
                param[i * 9 + j].toLowerCase() ==
                "0123456789wyorgbldmcpnk".charAt(k)
              ) {
                cube[i][j] = k;
                break;
              }
    ...
    if (param != null) {
      ...
      if (rm > 0) randMoveCount = rm;
    }
    // setup move sequence (and info texts)
    param = getParameter("move");
    if ("random" == param || scramble > 0) param = randMoves(3, randMoveCount);
    ...
    if (param != null) {
      ...
      var n = parseInt(param);
      if ((n >= 0) & (n <= 3)) align = n;
      if ((n >= 3) & (n <= 99)) align = n / 100;
    }
    param = getParameter("snap");
    if (param != null) if ("1" == param) snap = true;
    ...
    if (param != null && param.length == 6) {
      if (validateColor(param)) cubeColor = "#" + param;
    }
    moveCounter = 1;
  ...
  function getParameter(s) {
    ...
    if (typeof uparam == "undefined") return config[s];
    return uparam;
  }
  ...
  function getMove(sequence, info) {
    if (info) {
      ...
      if (infoText == null) {
        ...
        var infoText2 = [];
        for (var i = 0; i < infoText.length; i++) infoText2[i] = infoText[i];
  ...
  function getMovePart(sequence, info, num) {
    if (wcaNotation) sequence = wca_to_sign(sequence);
    if (sequence.trim() == "#")
      if (typeof move[num] != "undefined") return move[num];
    var length = 0;
    var mv = [];
    var moveCodeString = "UDFBLRESMXYZxyzudfblr";
    if (yzAlt == true) moveCodeString = "UDFBLRESMXZYxzyudfblr";
        ...
        for (var j = 0; j < 21; j++) {
          ...
          if (sequence.charAt(i) == moveCodeString.charAt(j)) {
            ...
            if (i < sequence.length) {
              if (moveModes[j] == 0) {
                // modifiers for basic characters UDFBLR
  ...
  function turnTextFunc(move, pos) {
    ...
    var s =
      turnSymbol[moveText - 1][Math.floor(move[pos] / 4) % 6][
        Math.floor(move[pos] / 24)
      ];
    if (s.charAt(0) == "~")
  ...
  function realMoveLength(move) {
    var length = 0;
    for (var i = 0; i < move.length; i++) if (move[i] < 1000) length++;
    return length;
  }

  function realMovePos(move, pos) {
    var rpos = 0;
    for (var i = 0; i < pos; i++) if (move[i] < 1000) rpos++;
    return rpos;
  }
  ...
  function turnLength(turn) {
    ...
    switch (metric) {
      ...
      case 3: // slice-turn metric
        if (mode == 3) n = 0;
        if (metric == 3 && (mode == 4 || mode == 5)) n *= 2;
    }
    return n;
  }

  function initInfoText(move) {
    if (move.length > 0 && move[0] >= 1000) curInfoText = move[0] - 1000;
    else curInfoText = -1;
  }
  ...
  function stopAnimation() {
    if (animating == true) restarted = true;
  }
  ...
  function clear() {
    ...
    if (
      ...
      typeof initialReversedMove[curMove] != "undefined"
    )
      doMove(
        cube,
    ...
    if (scramble > 0) move = getMove(randMoves(3, randMoveCount), false);
    if (scramble == 2) doMove(cube, move[0], 0, move[0].length, true);
    for (var i = 0; i < 3; i++) {
      eye[i] = initialEye[i];
  ...
  */
  ...
  function paint() {
    ...
    var h =
      buttonBar == 1 &&
      (progressHeight == 0 || demo || move[curMove].length == 0)
        ? height - dpr
        : height;
    ...
    if (natural) {
      ...
      for (var i = 0; i < 3; i++) {
        ...
        for (var j = 0; j < 3; j++) {
          ...
          tempEye[i] +=
            eye[j] *
            (rotVec[axis][i][j] +
              rotCos[axis][i][j] * cosA +
              rotSin[axis][i][j] * sinA);
          tempEyeX[i] +=
            eyeX[j] *
            (rotVec[axis][i][j] +
              rotCos[axis][i][j] * cosA +
      ...
      for (var i = 0; i < 3; i++) {
        ...
        for (var j = 0; j < 3; j++) {
          ...
          tempEye2[i] +=
            eye[j] *
            (rotVec[axis][i][j] +
              rotCos[axis][i][j] * cosB +
              rotSin[axis][i][j] * sinB);
          tempEyeX2[i] +=
            eyeX[j] *
            (rotVec[axis][i][j] +
              rotCos[axis][i][j] * cosB +
      ...
      vSub(
        vScale(vCopy(perspEye, eye), 5.0 + persp),
        vScale(vCopy(perspNormal, faceNormals[twistedLayer]), 1.0 / 3.0)
      );
      vSub(
        vScale(vCopy(perspEyeI, eye), 5.0 + persp),
    ...
    graphics.restore();
    if (drawButtons && buttonBar != 0)
      // omit unneccessary redrawing
  ...
  function fixBlock(eye, eyeX, eyeY, blocks, mode) {
    ...
    for (var i = 0; i < 8; i++) {
      ...
      else if (align == 1) coordsY[i] = (height - progressHeight) / 2.0 - y;
      else if (align == 2)
        coordsY[i] =
          height -
          progressHeight -
          ((height - progressHeight) / 2.0) * scale -
          y;
      else {
        // either equation works for 3-99
        coordsY[i] =
          (height - progressHeight) * (align * (1 - scale) + scale / 2) - y;
    ...
    if (hint) {
      // draw hint hidden facelets
      for (var i = 0; i < 6; i++) {
        // all faces
        vSub(vScale(vCopy(perspEye, eye), 5.0 + persp), faceNormals[i]); // perspective correction
        if (vProd(perspEye, faceNormals[i]) < -(1 - scale)) {
          // draw only hidden faces
    ...
    for (var i = 0; i < 6; i++) {
      ...
      var sideH = blocks[i][1][1] - blocks[i][1][0];
      if (sideW <= 0 || sideH <= 0) {
        // this face is inner and only black
        for (var j = 0; j < 4; j++) {
          // for all corners
          var k = oppositeCorners[i][j];
          fillX[j] = Math.floor(
            cooX[i][j] + ((cooX[i ^ 1][k] - cooX[i][j]) * 2.0) / 3.0
          );
          fillY[j] = Math.floor(
            cooY[i][j] + ((cooY[i ^ 1][k] - cooY[i][j]) * 2.0) / 3.0
          );
    ...
    for (var i = 0; i < 6; i++) {
      ...
      if (vProd(perspEye, faceNormals[i]) > -(1 - scale)) {
        ...
        if (mode == 3) {
          // just the normal cube
          for (var j = 0; j < 6; j++) {
            // 4 areas 3x1 per face + 2 center slices
            for (
              var k = 0;
              k < 4;
              k++ // 4 points per area
            ...
            dragDirsY[dragAreas] =
              (dyh * areaDirs[j][0] + dyv * areaDirs[j][1]) * twistDirs[i][j];
  ...
  function selectButton(x, y) {
    ...
    if (
      ...
      return 8;
    if (buttonBar == 2) {
      // only clear (rewind) button present
      if (
        x >= 0 &&
        x < buttonHeight &&
  ...
  function rgbToHex(r, g, b) {
    return (
      "#" + lpad(r.toString(16)) + lpad(g.toString(16)) + lpad(b.toString(16))
    );
  }
  ...
  function drawButtonsFunc(g) {
    ...
    adj += Math.floor(dpr + 0.5) - 1;
    if (buttonBar == 2) {
      // only clear (rewind) button
  ...
  function drawSuperArrow(g, xx, yy, face, superTwist, color) {
    var x = [];
    var y = [];
    var w = 0.26; // arrow width
    if (scw == 2 && color == "#ffffff") return;
    // scale down so there is a margin around the arrow
    for (var i = 0; i < 4; i++) {
      x[i] = Math.floor(xx[i] + (xx[superRotate[2][i]] - xx[i]) * 0.05);
      y[i] = Math.floor(yy[i] + (yy[superRotate[2][i]] - yy[i]) * 0.05);
    }
    if (face == 0) superTwist = (superTwist + 1) % 4;
    if (face == 4) superTwist = (superTwist + 3) % 4;
  ...
  function run(jobNum, dir) {
    ...
    if (!moveAnimated) {
      // fast-forward without using animation scheduler
      var mv = move[curMove];
      while (movePos < mv.length) {
        if (mv[movePos] >= 1000) curInfoText = mv[movePos] - 1000;
        ...
        if (!cont) {
          ...
          if (mv[movePos] == -1) {
            ...
            if (!moveOne) {
              ...
              sTime = Date.now();
              while (Date.now() - sTime < 1000); // empty loop
            }
      ...
      if (!cont) {
        if (spin) {
          if (moveAnimated) {
            if (currentAngle * phis < phit) {
              paint();
              if (interrupted || restarted) innerLoopBot = true;
              lTime = Date.now();
        ...
        if (innerLoopBot) {
          ...
          if (moveDir > 0) {
            ...
            if (movePos == mv.length) {
              if (!demo) outerLoopBot = true;
              else clearDemo(mv);
            }
          } else curInfoText = -1;
          if (interrupted || restarted || restart) outerLoopBot = true;
        } // innerLoopBot
      } // cont
      if (outerLoopBot) {
        outerLoopBot = false;
        outerLoopTop = true;
        if (jobNumber <= nowServing + 1) animating = false;
        drawButtons = true;
        if (buttonPressed == 0 || buttonPressed > 6) clear();
        paint();
        if (demo) {
          clear();
          demo = false;
        ...
        nowServing++;
        // cancel();
        if (movePos == 1 && mv[0] >= 1000) movePos--;
        return;
      }
      requestAnimationFrame(callback);
    }
  ...
  function touchfunc(s) {
    if (wrapDiv) divs[0].style.overflow = s;
    else document.body.style.overflow = s;
  }
  ...
  function mousedown(e) {
    ...
    if (typeof e.touches != "undefined") touchfunc("hidden");
    offsetX = left;
    offsetY = top;
    ...
    if (buttonPressed >= 0) button();
      ...
      if (typeof e.touches == "undefined") {
        ...
        if (editable && !animating) toTwist = true;
      }
    }
  }
  ...
  function button() {
    ...
    if (buttonPressed == 3) {
      ...
      // clear everything to the initial setup
      if (scramble > 0 && buttonBar == 2) {
      ...
      else curMove = curMove < move.length - 1 ? curMove + 1 : 0;
  ...
  function mousemove(e) {
    ...
    if (editable && toTwist && !twisting && !animating) {
      // we do not twist but we can
      lastDragX = x;
      lastDragY = y;
      for (var i = 0; i < dragAreas; i++) {
        // check if inside a drag area
  ...
  function contextmenu(e) {
    if (!showContextMenu) e.preventDefault();
  }
  ...
  function randMoves(s, n) {
    ...
    for (var i = 0; i < n; i++) {
      while (
        r1 == prev ||
        (Math.floor(r1 / 2) == Math.floor(prev / 2) && r1 == prev2)
      )
        r1 = Math.floor(Math.random() * 6);
      prev2 = prev;
      prev = r1;
      r2 = Math.floor(Math.random() * 3);
      if (s <= 3) randmoves += "" + face[r1] + twist[r2] + " ";
      else {
